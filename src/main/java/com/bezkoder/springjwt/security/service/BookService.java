...
//        courses.stream().filter(course -> course.getTeacher().getName().equals("a") ).collect(Collectors.toList());
		...
		courses.forEach(course -> {
			...
			books.forEach(book -> {
				BookDto bookDto = new BookDto(book.getId(), book.getISBN(), book.getTitle(), book.getPublisher(),
						book.getLanguage(), book.getNumOfPages(), book.getBookItems(), book.getAuthorList(),
						subjectCode);
